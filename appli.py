...
@app.put('/update_column_name', response_model=schema.Movie)
def update_column_name(sl_id: int, update_param:schema.UpdateColumnName, db: Session = Depends(get_db)):
    details = crud.get_movie_by_id(db=db, sl_id=sl_id)
    if not details:
        raise HTTPException(status_code=404, detail=f"No record found to update")
    
    return crud.update_column_name(db=db, details=update_column_name ,sl_id=sl_id)
